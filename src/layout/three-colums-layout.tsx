import { createUseStyles } from "react-jss";
import Render from "../components/render";
import { Column } from "../types/page";
const OneColumsLayout = ({ colums }: { colums: Column[] }) => {
  const style = usePageStyle();
  const [colum1, colum2, colum3] = colums;
  return (
    <div className={style.pageColums_3}>
      <AnchorPoint />
      <div />
      <AnchorPoint />
      <div />
      <AnchorPoint />
      <div />
      <AnchorPoint />
      <div />
      {colum1 != undefined ? <Render colum={colum1} /> : <div />}
      <div />
      {colum2 != undefined ? <Render colum={colum2} /> : <div />}
      <div />
      {colum3 != undefined ? <Render colum={colum3} /> : <div />}
      <div />
      <AnchorPoint />
      <div />
      <AnchorPoint />
      <div />
      <AnchorPoint />
      <div />
      <AnchorPoint />
    </div>
  );
};

const AnchorPoint = () => <div className="w-[20px] h-[20px] bg-black"></div>;

const usePageStyle = createUseStyles({
  pageColums_3: {
    height: "100%",
    padding: "20px",
    display: "grid",
    gridTemplateColumns: "20px 1fr 20px 1fr 20px 1fr 20px",
    gridTemplateRows: "20px 1fr 20px",
  },
});

export default OneColumsLayout;
